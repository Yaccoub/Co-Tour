...
def clean_data(data):
    rec_data = data.loc[:,
               ['neighbourhood', 'id', 'number_of_reviews', 'room_type', 'minimum_nights', 'price', 'last_review']]
    ...
    pred_data = rec_data.groupby(by='neighbourhood').agg(
        {'id': 'count', 'reviews_count': 'sum', 'price': 'mean'}).sort_values(by='price', ascending=True)
    # rename the columns
...
def main():
    months_listings = ['Mar19_listings', 'Mai19_listings', 'June19_listings', 'July19_listings', 'Aug19_listings',
                       'Sep19_listings', 'Oct19_listings', 'Nov19_listings', 'Dec19_listings', 'Jan20_listings',
                       'Feb20_listings', 'Mar20_listings', 'Apr20_listings', 'Mai20_listings', 'June20_listings']
    ret = []
    concat_dfs = []
    i = 1
