...
# rename the 'id','number of reviews' column for an intuitive use
Recommendation_Listings.rename(columns={'id':'accomodations_count','number_of_reviews':'reviews_count'}, inplace=True)
...
# list the data according to the neighbourhoods ; use only columns of : Nbr of accomodations / Nbr of reviews / price 
Prediction_Listings = Recommendation_Listings.groupby(by = 'neighbourhood').agg({'accomodations_count':'count', 'reviews_count':'sum','price':'mean'}).sort_values(by = 'price', ascending = True)
Prediction_Listings.rename(columns={'price':'avg_price'}, inplace=True)
